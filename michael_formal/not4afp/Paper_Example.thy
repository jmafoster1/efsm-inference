  ...
  show ?thesis
    apply (case_tac "aa = ''coin'' \<and> b = [Num 50]")
     ...
     apply (rule nondeterministic_simulates_trace.step_none)
      apply (simp add: drinks_before_1_invalid)
    ...
    apply (rule_tac x=coin50 in exI)
...
proof-
  have select_updates: "length b = 1 \<Longrightarrow> (\<lambda>x. if x = R 1 then aval (snd (R 1, V (I 1))) (case_vname (\<lambda>n. input2state b 1 (I n)) Map.empty)
          else EFSM.apply_updates [] (case_vname (\<lambda>n. input2state b 1 (I n)) Map.empty) Map.empty x) = <R 1 := hd b>"
    apply (rule ext)
...
lemma simulation_aux: "nondeterministic_simulates_trace merge_1_2 merge_1_2_update 0 0 Map.empty Map.empty t"
proof (induct t)
case Nil
  then show ?case
...
lemma simulation: "nondeterministic_simulates merge_1_2 merge_1_2_update"
