...
fun height :: "'a aexp \<Rightarrow> nat"  where
  ...
  "height (Divide e1 e2) = 1 + max (height e1) (height e2)"
...
fun less_aexp_aux :: "'a aexp \<Rightarrow> 'a aexp \<Rightarrow> bool"  where
  ...
  "less_aexp_aux (Minus e1 e2) (Divide e1' e2') = True" |
  ...
  "less_aexp_aux (Times e1 e2) (Divide e1' e2') = True" |
  "less_aexp_aux (Times e1 e2) _ = False" |

  "less_aexp_aux (Divide e1 e2) (Divide e1' e2') =  ((less_aexp_aux e1 e1') \<or> ((e1 = e1') \<and> (less_aexp_aux e2 e2')))" |
  "less_aexp_aux (Divide e1 e2) _ = False"
...
next
  ...
  then show ?case by (cases z, auto)
next
  case ("16_3" e1 e2 v va)
  then show ?case by (cases z, auto)
next
  case ("16_4" e1 e2 v va)
  then show ?case by (cases z, auto)
next
  case (17 e1 e2 e1' e2')
  then show ?case by (cases z, auto)
...
next
  ...
  then show ?case by (cases z, auto)
next
  case ("18_3" e1 e2 v va)
  then show ?case by (cases z, auto)
next
  case ("18_4" e1 e2 v va)
  then show ?case by (cases z, auto)
next
  case ("18_5" e1 e2 v va)
  then show ?case by (cases z, auto)
qed
