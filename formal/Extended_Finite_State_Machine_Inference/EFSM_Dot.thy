...
fun showsp_decimal :: "String.literal \<Rightarrow> real \<Rightarrow> nat \<Rightarrow> String.literal \<Rightarrow> String.literal"
where
  "showsp_decimal p n places =
    (if n = 0 then shows_string p else if places = 0 then shows_string STR ''...'' else
     showsp_decimal p (n*10 - (floor (n * 10))) (places - 1) o shows_string (show_int (floor (n * 10))))"
declare showsp_decimal.simps [simp del]

definition "rev_str s = String.implode (rev (String.explode s))"
code_printing constant rev_str \<rightharpoonup> (Scala) "(_.reverse)"

definition "show_decimal n p  \<equiv> rev_str (showsp_decimal ((STR '''')) n p ((STR ''.'')))"

definition "show_real (r::real) precision  \<equiv> showsp_int ((STR '''')) (floor r) (show_decimal (r - floor r) precision)"
...
fun value2dot :: "value \<Rightarrow> String.literal" where
  ...
  "value2dot (value.Int n) = show_int n" |
  "value2dot (value.Real n) = show_real n 3"
