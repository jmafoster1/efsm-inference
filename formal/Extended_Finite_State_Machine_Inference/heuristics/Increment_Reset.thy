...
definition initialiseReg :: "transition \<Rightarrow> nat \<Rightarrow> transition" where
  "initialiseReg t newReg = \<lparr>Label = Label t, Arity = Arity t, Guards = Guards t, Outputs = Outputs t, Updates = ((newReg, L (value.Int 0))#Updates t)\<rparr>"

definition "guardMatch t1 t2  = (\<exists>n n'. Guards t1 = [gexp.Eq (V (vname.I 0)) (L (value.Int n))] \<and> Guards t2 = [gexp.Eq (V (vname.I 0)) (L (value.Int n'))])"
definition "outputMatch t1 t2 = (\<exists>m m'. Outputs t1 = [L (value.Int m)] \<and> Outputs t2 = [L (value.Int m')])"
...
fun guardMatch_alt_2 :: "vname gexp list \<Rightarrow> bool" where
  "guardMatch_alt_2 [(gexp.Eq (V (vname.I i)) (L (value.Int n)))] = (i = 1)" |
  "guardMatch_alt_2 _ = False"

fun outputMatch_alt_2 :: "vname aexp list \<Rightarrow> bool" where
  "outputMatch_alt_2 [(L (value.Int n))] = True" |
