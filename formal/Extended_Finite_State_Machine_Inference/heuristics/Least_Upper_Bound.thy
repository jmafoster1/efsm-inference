...
proof(induct v)
  case (Int x)
  then show ?case
    by (metis (full_types, lifting) less_value.simps(1) less_value.simps(3) someI_ex)
next
  case (Real x)
  then show ?case
    by (metis (mono_tags, lifting) someI_ex value.distinct(6))
next
  case (Str x)
  then show ?case
    by (metis (mono_tags, lifting) someI_ex value.distinct(3))
qed
