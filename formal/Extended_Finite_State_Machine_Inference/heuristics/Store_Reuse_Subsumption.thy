...
lemma input_stored_in_reg_not_subsumed:
  ...
  subgoal for ia v
    ...
    subgoal for x1
      ...
      apply standard
      apply (rule_tac x="Eq (V (vname.I i)) (L (value.Int x1))" in exI)
      apply (simp add: join_ir_def input2state_nth is_generalisation_of_i_lt_arity)
    ...
    subgoal for x2
      ...
      apply (simp add: apply_guards_def Bex_def)
      apply standard
      apply (rule_tac x="Eq (V (vname.I i)) (L (value.Real x2))" in exI)
      by (simp add: join_ir_def input2state_nth is_generalisation_of_i_lt_arity str_not_num)
    subgoal for x3
      apply (rule_tac x="list_update ia i (value.Int _)" in exI)
      apply simp
      apply standard
       apply (simp add: apply_guards_def)
       apply (metis input_not_constrained_gval_swap_inputs)
      apply (simp add: apply_guards_def Bex_def)
      apply standard
      apply (rule_tac x="Eq (V (vname.I i)) (L (value.Str x3))" in exI)
      by (simp add: join_ir_def input2state_nth is_generalisation_of_i_lt_arity str_not_num)
    done
  done
