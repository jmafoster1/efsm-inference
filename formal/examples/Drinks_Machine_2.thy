...
lemma recognises_first_select: "recognises drinks 0 r ((aa, b) # as) \<Longrightarrow> aa = STR ''select'' \<and> length b = 1"
...
lemma recognises_1_2: "\<forall>r. recognises drinks 1 r t \<longrightarrow> recognises drinks2 2 r t"
proof(induct t)
  case Nil
  then show ?case
...
next
  ...
  then show ?case
    ...
    apply (case_tac "aa = STR ''coin'' \<and> length b = 1")
     apply (simp add: recognises_cons)
    ...
    apply (case_tac "x1 \<ge> 100")
     defer
     apply (simp add: recognises_cons drinks_vend_insufficient drinks2_vend_insufficient2)
    apply simp
    apply (simp add: recognises_cons drinks_vend_sufficient drinks2_vend_sufficient)
    apply (cases as)
     apply (simp add: recognises.base)
    using drinks_rejects_future by blast
qed

lemma recognises_quantified: "\<forall>s r. recognises drinks s r t \<longrightarrow> recognises drinks2 s r t"
proof(induct t)
  case Nil
  then show ?case
    by (simp add: recognises.base)
next
  ...
  then show ?case
    ...
    using recognises_cons first_step_select apply fastforce
    using recognises_first_select apply auto[1]
    apply (case_tac "s=1")
     apply (case_tac "aa = STR ''coin'' \<and> length b = 1")
      apply (simp add: recognises_cons possible_steps_1_coin possible_steps_1 recognises_1_2)
     apply (case_tac "aa = STR ''vend'' \<and> b = []")
      defer
      apply (simp add: drinks_1_rejects_trace)
     apply (case_tac "s=2")
    ...
    apply (case_tac "x1 \<ge> 100")
     defer
     apply (simp add: recognises_cons drinks_vend_insufficient drinks2_vend_insufficient vend_nothing_def vend_fail_def)
    apply simp
    apply (simp add: recognises_cons drinks_vend_sufficient drinks2_vend_insufficient)
    by (metis recognises.simps drinks_rejects_future)
qed

lemma acceptance: "recognises drinks s r t \<Longrightarrow> recognises drinks2 s r t"
  using recognises_quantified by blast
...
lemma purchase_coke: "observe_trace drinks2 0 <> [((STR ''select''), [Str ''coke'']), ((STR ''coin''), [Num 50]), ((STR ''coin''), [Num 50]), ((STR ''vend''), [])] =
  ...
  apply (rule observe_trace_possible_step)
     ...
     apply (simp add: coin_def value_plus_def join_ir_def input2state_def regsimp recognises_from_2 recognises_1_2)
    apply (simp add: coin_def value_plus_def join_ir_def input2state_def regsimp apply_outputs_def)
   apply (simp add: coin_def value_plus_def join_ir_def input2state_def regsimp)
  apply (rule observe_trace_possible_step)
...
lemma rejects_1_2: "\<forall>r. \<not> recognises drinks 1 r t \<longrightarrow> \<not> recognises drinks2 2 r t"
proof(induct t)
  case Nil
  then show ?case
    by (simp add: recognises.base)
next
  ...
  then show ?case
    apply (cases a)
    apply (case_tac "aa = STR ''coin'' \<and> length b = 1")
     apply (simp add: recognises_cons possible_steps_1_coin possible_steps_2_coin)
    apply (case_tac "aa = STR ''vend'' \<and> b = []")
     defer
    using recognises_possible_steps_not_empty drinks2_2_invalid apply fastforce
    apply (rule allI)
    apply (case_tac "r $ 2")
    ...
    apply (case_tac "x1 \<ge> 100")
     ...
     apply simp
     apply standard
    using recognises_possible_steps_not_empty drinks2_end apply blast
    by (simp add: trace_reject_2 drinks_vend_insufficient drinks2_vend_insufficient2)
qed


lemma recognises_1_1: "\<not> recognises drinks 1 (<>(1 := d, 2 := Num 0)) t \<Longrightarrow> \<not> recognises drinks2 1 (<>(1 := d, 2 := Num 0)) t"
proof(induct t)
  case Nil
  then show ?case
    by (simp add: recognises.base)
