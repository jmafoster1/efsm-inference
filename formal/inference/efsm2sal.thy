...
fun gexp2sal :: "gexp \<Rightarrow> String.literal" where
  "gexp2sal (gexp.Bc True) = STR ''True''" |
  "gexp2sal (gexp.Bc False) = STR ''False''" |
  "gexp2sal (gexp.Eq a1 a2) = STR ''gval(value_eq('' + aexp2sal a1 + STR '', '' + aexp2sal a2 + STR ''))''" |
  "gexp2sal (gexp.Gt a1 a2) = STR ''gval(value_gt('' + aexp2sal a1 + STR '', '' + aexp2sal a2 + STR ''))''" |
  "gexp2sal (gexp.In v l) = join (map (\<lambda>l'.  STR ''gval(value_eq('' + aexp2sal (V v) + STR '', '' + aexp2sal (L l') + STR ''))'') l) STR '' OR ''" |
  "gexp2sal (gexp.Nor g1 g2) = STR ''NOT (gval('' + gexp2sal g1 + STR '') OR gval( '' + gexp2sal g2 + STR ''))''" |
  "gexp2sal (gexp.Null a1) = STR ''a1 = None''"
