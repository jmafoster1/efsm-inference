...
class to_string = fixes toString  :: "'a \<Rightarrow> String.literal" ("toString _" [40] 40)
...
fun aexp2dot :: "('a::to_string, 'b::to_string) aexp \<Rightarrow> String.literal" where
  "aexp2dot (L v) = (toString v)" |
  "aexp2dot (V v) = (toString v)" |
...
fun gexp2dot :: "('a::to_string, 'b::to_string) gexp \<Rightarrow> String.literal" where
  ...
  "gexp2dot (GExp.In v l) = (toString v)+STR ''&isin;{''+(join (map (\<lambda>x. toString x) l) STR '', '')+STR ''}''" |
  "gexp2dot (Nor g1 g2) = STR ''!(''+(gexp2dot g1)+STR ''&or;''+(gexp2dot g2)+STR '')''"

primrec guards2dot_aux :: "('a::to_string, 'b::to_string) gexp list \<Rightarrow> String.literal list" where
  "guards2dot_aux [] = []" |
...
instantiation "value" :: to_string begin
  fun toString_value :: "value \<Rightarrow> String.literal" where
    "toString_value (value.Str s) = quote + replace_backslash s + quote" |
    "toString_value (Num n) = show_int n"

  instance by standard
end

instantiation vname :: to_string begin
  fun toString_vname :: "vname \<Rightarrow> String.literal" where
    "toString_vname (vname.I n) = STR ''i<sub>''+(show_nat (n))+STR ''</sub>''" |
    "toString_vname (R n) = STR ''r<sub>''+(show_nat n)+STR ''</sub>''"

  instance by standard
end

primrec updates2dot_aux :: "('a::to_string) update_function list \<Rightarrow> String.literal list" where
  "updates2dot_aux [] = []" |
  "updates2dot_aux (h#t) = ((toString (R (fst h)))+STR '' := ''+(aexp2dot (snd h)))#(updates2dot_aux t)"

lemma updates2dot_aux_code [code]:
  "updates2dot_aux l = map (\<lambda>(r, u). (toString (R r))+STR '' := ''+(aexp2dot u)) l"
  by (induct l, auto)

primrec outputs2dot :: "('a::to_string) output_function list \<Rightarrow> nat \<Rightarrow> String.literal list" where
  "outputs2dot [] _ = []" |
  "outputs2dot (h#t) n = ((STR ''o<sub>''+(show_nat n))+STR ''</sub> := ''+(aexp2dot h))#(outputs2dot t (n+1))"

fun updates2dot :: "('a::to_string) update_function list \<Rightarrow> String.literal" where
  "updates2dot [] = (STR '''')" |
  "updates2dot a = STR ''&#91;''+(join (updates2dot_aux a) STR '', '')+STR ''&#93;''"

fun guards2dot :: "('a::to_string, 'b::to_string) gexp list \<Rightarrow> String.literal" where
  "guards2dot [] = (STR '''')" |
  "guards2dot a = STR ''&#91;''+(join (guards2dot_aux a) STR '', '')+STR ''&#93;''"

definition latter2dot :: "('a::to_string) transition \<Rightarrow> String.literal" where
  "latter2dot t = (let l = (join (outputs2dot (Outputs t) 1) STR '', '')+(updates2dot (Updates t)) in (if l = (STR '''') then (STR '''') else STR ''/''+l))"

definition transition2dot :: "('a::to_string) transition \<Rightarrow> String.literal" where
  "transition2dot t = (Label t)+STR '':''+(show_nat (Arity t))+(guards2dot (Guards t))+(latter2dot t)"

definition efsm2dot :: "('a::{to_string,linorder}) transition_matrix \<Rightarrow> String.literal" where
...
definition iefsm2dot :: "('a::{to_string,linorder}) iEFSM \<Rightarrow> String.literal" where
