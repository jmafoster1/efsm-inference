...
fun everything_walk :: "transition option \<Rightarrow> output_function \<Rightarrow> nat \<Rightarrow> (vname \<Rightarrow>f String.literal) \<Rightarrow> execution \<Rightarrow> iEFSM \<Rightarrow> cfstate \<Rightarrow> registers \<Rightarrow> tids list \<Rightarrow> run_info" where
  "everything_walk _ _ _ _ [] _ _ _ _ = []" |
  "everything_walk prev f fi types ((label, inputs, outputs)#t) oPTA s regs gp  = (
    let (tid, s', ta) = fthe_elem (i_possible_steps oPTA s regs label inputs) in
     \<comment> \<open>Possible steps with a transition we need to modify\<close>
    if tid \<in> set gp then
      (s, regs, get_regs types inputs f (outputs!fi), inputs, tid, ta)#(everything_walk (Some ta) f fi types t oPTA s' (apply_updates (Updates ta) (join_ir inputs regs) regs) gp)
    else
      let empty = <> in
      (s, regs, empty, inputs, tid, ta)#(everything_walk (Some ta) f fi types t oPTA s' (apply_updates (Updates ta) (join_ir inputs regs) regs) gp)
  )"
...
definition everything_walk_log :: "output_function \<Rightarrow> nat \<Rightarrow> (vname \<Rightarrow>f String.literal) \<Rightarrow> log \<Rightarrow> iEFSM \<Rightarrow> tids list \<Rightarrow> run_info list" where
  "everything_walk_log f fi types log e gp = map (\<lambda>t. everything_walk None f fi types t e 0 <> gp) log"
