...
'as and replace two transitions whose outputs differ with a single transition that outputs the
...
fun lookup :: "index \<Rightarrow> 'a trace \<Rightarrow> 'a" where
  "lookup (actionNo, In, inx) t = (let (_, inputs, _) = nth t actionNo in inputs ! inx)" |
  "lookup (actionNo, Out, inx) t = (let (_, _, outputs) = nth t actionNo in outputs ! inx)"
...
primrec index :: "'a list \<Rightarrow> nat \<Rightarrow> ioTag \<Rightarrow> nat \<Rightarrow> index fset" where
  "index [] _ _ _ = {||}" |
  "index (h#t) actionNo io ind = finsert (actionNo, io, ind) (index t actionNo io (ind + 1))"

definition io_index :: "nat \<Rightarrow> 'a list \<Rightarrow> 'a list \<Rightarrow> index fset" where
  "io_index actionNo inputs outputs = (index inputs actionNo In 0) |\<union>| (index outputs actionNo Out 0)"

definition indices :: "'a trace \<Rightarrow> index fset" where
  "indices e = foldl (|\<union>|) {||} (map (\<lambda>(actionNo, (label, inputs, outputs)). io_index actionNo inputs outputs) (enumerate 0 e))"

definition get_by_id_intratrace_matches :: "'a trace \<Rightarrow> (index \<times> index) fset" where
...
(*
  ...
  the input 'a trace.
*)
definition i_step :: "('a::aexp_value) execution \<Rightarrow> 'a iEFSM \<Rightarrow> cfstate \<Rightarrow>'a registers \<Rightarrow> label \<Rightarrow>'a inputs \<Rightarrow> ('a transition \<times> cfstate \<times> tids \<times>'a registers) option" where
...
type_synonym 'a match = "((('a transition \<times> tids) \<times> ioTag \<times> nat) \<times> (('a transition \<times> tids) \<times> ioTag \<times> nat))"

definition "exec2trace t = map (\<lambda>(label, inputs, _). (label, inputs)) t"
primrec (nonexhaustive) walk_up_to :: "nat \<Rightarrow> ('a::aexp_value) iEFSM \<Rightarrow> nat \<Rightarrow>'a registers \<Rightarrow> 'a trace \<Rightarrow> ('a transition \<times> tids)" where
...
definition find_intertrace_matches_aux :: "(index \<times> index) fset \<Rightarrow> ('a::{aexp_value,order}) iEFSM \<Rightarrow> 'a trace \<Rightarrow> 'a match fset" where
...
definition find_intertrace_matches :: "('a::{aexp_value,linorder}) log \<Rightarrow> 'a iEFSM \<Rightarrow> 'a match list" where
...
definition total_max_input :: "('a::{aexp_value,order}) iEFSM \<Rightarrow> nat" where
  "total_max_input e = (case EFSM.max_input (tm e) of None \<Rightarrow> 0 | Some i \<Rightarrow> i)"

definition total_max_reg :: "('a::{aexp_value,order}) iEFSM \<Rightarrow> nat" where
  "total_max_reg e = (case EFSM.max_reg (tm e) of None \<Rightarrow> 0 | Some i \<Rightarrow> i)"

definition remove_guard_add_update :: "('a::{aexp_value,order}) transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> 'a transition" where
...
definition generalise_output :: "('a::{aexp_value,order}) transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> 'a transition" where
...
definition is_generalised_output_of :: "('a::{aexp_value,order}) transition \<Rightarrow> 'a transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> bool" where
...
definition replaceAll :: "'a iEFSM \<Rightarrow> 'a transition \<Rightarrow> 'a transition \<Rightarrow> 'a iEFSM" where
  "replaceAll e old new = fimage (\<lambda>(uid, (from, dest), t). if t = old then (uid, (from, dest), new) else (uid, (from, dest), t)) e"

primrec generalise_transitions :: "(((('a transition \<times> tids) \<times> ioTag \<times> nat) \<times> ('a transition \<times> tids) \<times> ioTag \<times> nat) \<times>
     (('a transition \<times> tids) \<times> ioTag \<times> nat) \<times> ('a transition \<times> tids) \<times> ioTag \<times> nat) list \<Rightarrow> 'a iEFSM \<Rightarrow> 'a iEFSM" where
...
definition strip_uids :: "((('a transition \<times> tids) \<times> ioTag \<times> nat) \<times> ('a transition \<times> tids) \<times> ioTag \<times> nat) \<Rightarrow> (('a transition \<times> ioTag \<times> nat) \<times> ('a transition \<times> ioTag \<times> nat))" where
  "strip_uids x = (let (((t1, u1), io1, in1), (t2, u2), io2, in2) = x in ((t1, io1, in1), (t2, io2, in2)))"

definition modify :: "('a::{aexp_value,order}) match list \<Rightarrow> tids \<Rightarrow> tids \<Rightarrow> 'a iEFSM \<Rightarrow> 'a iEFSM option" where
...
(* type_synonym update_modifier = "transition \<Rightarrow> transition \<Rightarrow> nat \<Rightarrow> 'a iEFSM \<Rightarrow> 'a iEFSM \<Rightarrow> (iEFSM \<times> (nat \<Rightarrow> nat) \<times> (nat \<Rightarrow> nat)) option" *)
definition heuristic_1 :: "('a::{aexp_value,linorder}) log \<Rightarrow> 'a update_modifier" where
...
definition is_generalisation_of :: "('a::{aexp_value,order}) transition \<Rightarrow> 'a transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> bool" where
...
definition is_proper_generalisation_of :: "('a::{aexp_value,order}) transition \<Rightarrow> 'a transition \<Rightarrow> 'a iEFSM \<Rightarrow> bool" where
...
definition generalise_input :: "'a transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> 'a transition" where
...
fun structural_count :: "(('a transition \<times> ioTag \<times> nat) \<times> ('a transition \<times> ioTag \<times> nat)) \<Rightarrow> (('a transition \<times> ioTag \<times> nat) \<times> ('a transition \<times> ioTag \<times> nat)) list \<Rightarrow> nat" where
...
definition remove_guards_add_update :: "'a transition \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> 'a transition" where
...
definition modify_2 :: "'a match list \<Rightarrow> tids \<Rightarrow> tids \<Rightarrow> 'a iEFSM \<Rightarrow> 'a iEFSM option" where
...
(* type_synonym update_modifier = "transition \<Rightarrow> transition \<Rightarrow> nat \<Rightarrow> 'a iEFSM \<Rightarrow> 'a iEFSM \<Rightarrow> (iEFSM \<times> (nat \<Rightarrow> nat) \<times> (nat \<Rightarrow> nat)) option" *)
definition heuristic_2 :: "('a::{aexp_value,linorder}) log \<Rightarrow> 'a update_modifier" where
