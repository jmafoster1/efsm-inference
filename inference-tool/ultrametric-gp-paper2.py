...
runtime_re = re.compile(r"Completed in (\d+)h (\d+)m (\d+).\d+s")

homedir = "/home/michael/Documents/efsm-inference/inference-tool/results"
# homedir = "/home/michael/Documents/results"

graphsdir = f"{homedir}/graphs2"

data = {"liftDoors": {}, "spaceInvaders": {}, "shopScanners": {}}
...
def box1(column, program, cfgs, fname, title, ax1):
    ...
    labels = [
        " ".join(
            c.replace("obfuscated", "obfuscated\n")
            .replace("pta", "PTA")
            .replace("gp", "GP")
            .split("-")
        )
    ...
    ax1.set_xticklabels(
        labels, rotation=30, ha="right", va="top", ma="right", rotation_mode="anchor"
    )
...
def boxPlots(column, ps, fname, systems):
    ...
    # plt.tight_layout()
    plt.savefig(f"{graphsdir}/{fname}.pdf", bbox_inches="tight")
    plt.close()
    print(
        "Saving to", f"{graphsdir}/{fname}.pdf",
    )
    return boxes


if not os.path.exists(f"{graphsdir}"):
    os.mkdir(f"{graphsdir}")
...
fname = "".join([p[:2] for p in data])

ps = {p: configs(p) for p in data}

ls, ss = boxPlots("sensitivity", ps, f"{fname}-sensitivity-plots", data)
lt, st = boxPlots("t1", ps, f"{fname}-t1-plots", data)
lt, st = boxPlots("states", ps, f"{fname}-states-plots", data)
lt, st = boxPlots("transitions", ps, f"{fname}-transitions-plots", data)
lt, st = boxPlots("runtime", ps, f"{fname}-runtime-plots", data)
...
ax3.set_xticklabels(
    [
        " ".join(
            c.replace("obfuscated", "obfuscated\n")
            .replace("pta", "PTA")
            .replace("none", "without\npreprocessing")
            .replace("distinguish", "\nguards")
            .replace("gp", "GP")
            .split("-")
        )
        for c in configs("liftDoors")
    ],
    rotation=30,
    ha="right",
    va="top",
...
ax4.set_xticklabels(
    [
        ...
        for c in configs("spaceInvaders")
    ],
    rotation=30,
    ha="right",
    va="top",
...
# plt.savefig("/home/michael/Documents/papers/efsm-gp2/figures/graphs/lisp-plots.pdf", bbox_inches='tight')
