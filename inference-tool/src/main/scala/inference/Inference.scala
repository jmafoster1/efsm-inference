...
object Symbolic_Regression {
  ...
  def everything_walk(uu: AExp.aexp, uv: Nat.nat,
    x2: List[(String, (List[Value.value], List[Value.value]))],
    uw: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ux: Nat.nat, uy: Map[Nat.nat, Option[Value.value]],
    uz: String, va: Nat.nat): List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit]))))] =
    (uu, uv, x2, uw, ux, uy, uz, va) match {
      case (uu, uv, Nil, uw, ux, uy, uz, va) => Nil
      case (f, fi, (label, (inputs, outputs)) :: t, oPTA, s, regs, ll, i_arity) =>
        {
          val (tid, (sa, ta)): (List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit])) = FSet.fthe_elem[(List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit]))](Inference.i_possible_steps(oPTA,
            s, regs, label, inputs));
          (if ((ll == label) && (Nat.equal_nata(Nat.Nata(inputs.length), i_arity)))
            (s, (Dirties.getRegs(inputs, f,
            outputs(Code_Numeral.integer_of_nat(fi).toInt)),
            (inputs, (tid, ta)))) ::
            everything_walk(f, fi, t, oPTA, sa,
              EFSM.apply_updates(Transition.Updates[Unit](ta),
  ...
  def everything_walk_log(f: AExp.aexp, fi: Nat.nat,
    ...
    Lista.map[List[(String, (List[Value.value], List[Value.value]))], List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit]))))]](((t: List[(String, (List[Value.value], List[Value.value]))]) =>
      everything_walk(f, fi, t, e, Nat.zero_nata,
                ...
                case Some(prevtida) =>
                  {
                    ...
                    (if (FSet.fBex[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](newE,
                      ...
                      None
                    else put_output_function_aux(fi, f, t, label,
          ...
          case Some(prevtida) =>
            {
              ...
              (if (!(Nat.equal_nata(Nat.Nata(necessaryRegs.length),
                ...
                val newE: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = Inference.replace_transitions(e,
                  List((tid, newT), (prevtida, newPrevT)));
                put_output_function_2_aux(fi, f, t, label, i_arity,
                  o_arity, Some[List[Nat.nat]](tid), newE, sa,
                  EFSM.apply_updates(Transition.Updates[Unit](ta),
                    AExp.join_ir(i, r), r))
              })
            }
        })
        else put_output_function_2_aux(fi, f, t, label, i_arity, o_arity,
          Some[List[Nat.nat]](tid), e, sa,
          EFSM.apply_updates(Transition.Updates[Unit](ta), AExp.join_ir(i, r), r)))
      }
    }
    ...
    uv: (FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], List[Nat.nat]), (Transition.transition_ext[Unit], List[Nat.nat]))))]): Option[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      val i_log: List[(Nat.nat, List[(Nat.nat, (String, (List[Value.value], List[Value.value])))])] = Lista.enumerate[List[(Nat.nat, (String, (List[Value.value], List[Value.value])))]](Nat.zero_nata,
        Lista.map[List[(String, (List[Value.value], List[Value.value]))], List[(Nat.nat, (String, (List[Value.value], List[Value.value])))]](((a: List[(String, (List[Value.value], List[Value.value]))]) =>
          ...
          log))
      val t1: Transition.transition_ext[Unit] = Inference.get_by_ids(newa, t1ID);
      ((a: AExp.aexp) => (b: Nat.nat) =>
        (c: List[List[(String, (List[Value.value], List[Value.value]))]]) =>
          (d: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) =>
        ...
        i_log, t1, pta) match {
          ...
          case Some(lit) =>
            (output_functions.head match {
              ...
              case Some(op) =>
                {
                  val walked: List[List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit]))))]] = everything_walk_log(op, Nat.zero_nata, log, lit,
                    Transition.Label[Unit](t1), Transition.Arity[Unit](t1));
                  val targeted = Lista.map[List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit]))))], List[(Map[Nat.nat, Option[Value.value]], (Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit])))))]](((w: List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit]))))]) =>
