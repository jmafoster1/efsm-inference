...
object Symbolic_Regression {
  ...
  def everything_walk(uu: Option[Transition.transition_ext[Unit]], uv: AExp.aexp,
    uw: Nat.nat, ux: Map[VName.vname, String],
    x4: List[(String, (List[Value.value], List[Value.value]))],
    uy: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    uz: Nat.nat, va: Map[Nat.nat, Option[Value.value]],
    vb: List[List[Nat.nat]]): List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit])))))] =
    (uu, uv, uw, ux, x4, uy, uz, va, vb) match {
      case (uu, uv, uw, ux, Nil, uy, uz, va, vb) => Nil
      case (prev, f, fi, types, (label, (inputs, outputs)) :: t, oPTA, s, regs, gp) => {
        val (tid, (sa, ta)): (List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit])) = FSet.fthe_elem[(List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit]))](Inference.i_possible_steps(oPTA,
          s, regs, label, inputs));
        (if (gp contains tid) {
          println(outputs + s"[${PrettyPrinter.show(fi)}]")
          (s, (regs,
          (Dirties.getRegs(types, inputs, f,
            outputs(Code_Numeral.integer_of_nat(fi).toInt)),
            (inputs, (tid, ta))))) ::
            everything_walk(Some[Transition.transition_ext[Unit]](ta), f, fi,
              types, t, oPTA, sa,
              EFSM.apply_updates(Transition.Updates[Unit](ta),
          ...
          (s, (regs, (empty, (inputs, (tid, ta))))) ::
            everything_walk(Some[Transition.transition_ext[Unit]](ta),
              f, fi, types, t, oPTA, sa,
              EFSM.apply_updates(Transition.Updates[Unit](ta),
                AExp.join_ir(inputs, regs), regs),
              gp)
        })
  ...
  def same_structure_opt[A](x0: Option[Transition.transition_ext[A]],
    x1: Option[Transition.transition_ext[A]]): Boolean =
    (x0, x1) match {
      case (None, None) => true
    ...
    def everything_walk_log(
      ...
      gp: List[List[Nat.nat]]): List[List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit])))))]] = {
        ...
        // println("Group output function: " + PrettyPrinter.aexpToString(f))
        // gp.map(id => println(PrettyPrinter.show(Inference.get_by_ids(e, id))))
        ...
        Lista.map[List[(String, (List[Value.value], List[Value.value]))], List[(Nat.nat, (Map[Nat.nat, Option[Value.value]], (Map[Nat.nat, Option[Value.value]], (List[Value.value], (List[Nat.nat], Transition.transition_ext[Unit])))))]](((t: List[(String, (List[Value.value], List[Value.value]))]) =>
          everything_walk(None, f, fi, types, t, e,
            Nat.zero_nata, Map().withDefaultValue(None), gp)),
          log)
      }
...
object PTA_Generalisation {
  ...
  def groupwise_put_updates(
    x0: List[List[(List[Nat.nat], Transition.transition_ext[Unit])]],
    uu: List[List[(String, (List[Value.value], List[Value.value]))]],
    uv: List[Value.value],
    uw: (Nat.nat, (AExp.aexp, Map[VName.vname, String])),
    e: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] =
    (x0, uu, uv, uw, e) match {
      case (Nil, uu, uv, uw, e) => e
      case (gp :: gps, log, values, (o_inx, (op, types)), e) =>
        {
          println()
          println("Updating group:")
          gp.map{case (_, t) => println("  " + PrettyPrinter.show(t))}
          ...
          groupwise_put_updates(gps, log, values, (o_inx, (op, types)), a)
        }
    }
        ...
        (if (Lista.list_all[(List[Nat.nat], Transition.transition_ext[A])](((a: (List[Nat.nat], Transition.transition_ext[A])) =>
          {
            ...
            Symbolic_Regression.same_structure[A](pair._2, aa)
          }),
          h))
  ...
  def assign_group(x0: List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])],
    k: Option[Transition.transition_ext[Unit]],
    v: (List[Nat.nat], Transition.transition_ext[Unit])): List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])] =
    (x0, k, v) match {
      case (Nil, k, v) => List((k, List(List(v))))
      case ((ka, groups) :: t, k, v) =>
        (if (Symbolic_Regression.same_structure_opt[Unit](ka, k))
          (ka, insert_into_group[Unit](groups, v)) :: t
        else (ka, groups) :: assign_group(t, k, v))
    }

  def trace_group_transitions(uu: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    x1: List[(String, (List[Value.value], List[Value.value]))],
    uv: Nat.nat, uw: Map[Nat.nat, Option[Value.value]],
    ux: Option[Transition.transition_ext[Unit]],
    g: List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])]): List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])] =
    (uu, x1, uv, uw, ux, g) match {
      case (uu, Nil, uv, uw, ux, g) => g
      case (e, (l, (i, uy)) :: trace, s, r, prev, g) =>
        {
          val (id, (sa, t)): (List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit])) = FSet.fthe_elem[(List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit]))](Inference.i_possible_steps(e,
            s, r, l, i))
          val ra: Map[Nat.nat, Option[Value.value]] =
            EFSM.apply_updates(Transition.Updates[Unit](t), AExp.join_ir(i, r), r)
          val a: List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])] = assign_group(g, prev, (id, t));
          trace_group_transitions(e, trace, sa, ra,
            Some[Transition.transition_ext[Unit]](t), a)
        }
    }

  def log_group_transitions(e: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    l: List[List[(String, (List[Value.value], List[Value.value]))]]): List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])] =
    Lista.fold[List[(String, (List[Value.value], List[Value.value]))], List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])]](((t: List[(String, (List[Value.value], List[Value.value]))]) =>
      ((a: List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])]) =>
        trace_group_transitions(e, t, Nat.zero_nata,
          Map().withDefaultValue(None), None, a))),
      l, Nil)
  ...
  def transition_groups(e: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    {
      val group_dict: List[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])] = log_group_transitions(e, l);
      Lista.fold[(Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]]), List[List[(List[Nat.nat], Transition.transition_ext[Unit])]]](((a: (Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]])) =>
        {
          val (_, v): (Option[Transition.transition_ext[Unit]], List[List[(List[Nat.nat], Transition.transition_ext[Unit])]]) = a;
          ((acc: List[List[(List[Nat.nat], Transition.transition_ext[Unit])]]) =>
            Lista.union[List[(List[Nat.nat], Transition.transition_ext[Unit])]].apply(acc).apply(v))
        }),
        group_dict, Nil)
    }
  ...
  def put_updates(uu: List[List[(String, (List[Value.value], List[Value.value]))]],
    uv: List[Value.value],
    x2: List[(Nat.nat, Option[(AExp.aexp, Map[VName.vname, String])])],
    e: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Option[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    (uu, uv, x2, e) match {
      case (uu, uv, Nil, e) =>
        Some[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](e)
      case (uw, ux, (uy, None) :: uz, va) => None
      case (log, values, (o_inx, Some((op, types))) :: ops, e) =>
        (if (!(Cardinality.eq_set[Nat.nat](AExp.enumerate_aexp_regs(op),
          Set.bot_set[Nat.nat]))) {
          val groups: List[List[(List[Nat.nat], Transition.transition_ext[Unit])]] = transition_groups(e, log)
          val a: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = groupwise_put_updates(groups, log, values, (o_inx, (op, types)), e);
          put_updates(log, values, ops, a)
        } else put_updates(log, values, ops, e))
    }
  ...
  def generalise_and_update(
    log: List[List[(String, (List[Value.value], List[Value.value]))]],
    e: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    tt: List[(List[Nat.nat], Transition.transition_ext[Unit])],
    train: List[(Map[Nat.nat, Option[Value.value]], (List[Value.value], List[Value.value]))]
  ): Option[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      for ((id, t) <- tt)
        println(PrettyPrinter.show(id), PrettyPrinter.show(t))
      println()
  ...
  def groupwise_generalise_and_update(uu: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    (uu, e, x2) match {
      case (uu, e, Nil) => e
      case (log, e, (tt, train) :: t) =>
        (generalise_and_update(log, e, tt, train) match {
  ...
  def incremental_normalised_pta(pta: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    log: List[List[(String, (List[Value.value], List[Value.value]))]]): FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] =
    {
  ...
  def derestrict(log: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    {
      val pta: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = Inference.make_pta(log)
      val normalised: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = incremental_normalised_pta(pta, log)
