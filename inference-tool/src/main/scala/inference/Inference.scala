...
object HOL {
  ...
  object equal {
    ...
    implicit def `Type_Inference.equal_type`: equal[Type_Inference.typea] = new equal[Type_Inference.typea] {
    ...
    implicit def `Lista.equal_list`[A: equal]: equal[List[A]] = new equal[List[A]] {
      val `HOL.equal` = (a: List[A], b: List[A]) => a == b
    }
...
object Orderings {
  ...
  object ord {
    ...
    implicit def `GExp_Orderings.ord_option`[A: HOL.equal: linorder]: ord[Option[A]] = new ord[Option[A]] {
  ...
  object preorder {
    ...
    implicit def `GExp_Orderings.preorder_option`[A: HOL.equal: linorder]: preorder[Option[A]] = new preorder[Option[A]] {
      val `Orderings.less_eq` = (a: Option[A], b: Option[A]) =>
        GExp_Orderings.less_eq_option[A](a, b)
      val `Orderings.less` = (a: Option[A], b: Option[A]) =>
        GExp_Orderings.less_option[A](a, b)
  ...
  object order {
    ...
    implicit def `GExp_Orderings.order_option`[A: HOL.equal: linorder]: order[Option[A]] =
      new order[Option[A]] {
        val `Orderings.less_eq` = (a: Option[A], b: Option[A]) =>
          GExp_Orderings.less_eq_option[A](a, b)
        val `Orderings.less` = (a: Option[A], b: Option[A]) =>
  ...
  object linorder {
    ...
    implicit def `GExp_Orderings.linorder_option`[A: HOL.equal: linorder]: linorder[Option[A]] = new linorder[Option[A]] {
      val `Orderings.less_eq` = (a: Option[A], b: Option[A]) =>
        GExp_Orderings.less_eq_option[A](a, b)
      val `Orderings.less` = (a: Option[A], b: Option[A]) =>
        GExp_Orderings.less_option[A](a, b)
  ...
  def less_eq_set[A: HOL.equal](a: set[A], b: set[A]): Boolean = (a, b) match {
    case (coset(xs), seta(ys)) =>
      (if ((xs.isEmpty) && (ys.isEmpty)) false
        ...
        sys.error("subset_eq (List.coset _) (List.set _) requires type class instance card_UNIV");
        (((_: Unit) =>
...
object Lista {
  ...
  def foldl[A, B](f: A => B => A, s: A, xs: List[B]): A =
    fold[B, A](((x: B) => (sa: A) => (f(sa))(x)), xs, s)

  def foldr[A, B](f: A => B => B, xs: List[A], a: B): B =
    foldl[B, A](((x: B) => (y: A) => (f(y))(x)), a, xs.reverse)
  ...
  def sorted_list_of_set[A: HOL.equal: Orderings.linorder](x0: Set.set[A]): List[A] =
    x0 match {
...
object AExp {
  ...
  def value_minus(uu: Option[Value.value], uv: Option[Value.value]): Option[Value.value] =
    (uu, uv) match {
  ...
  def value_plus(uu: Option[Value.value], uv: Option[Value.value]): Option[Value.value] =
    (uu, uv) match {
      case (Some(Value.Numa(x)), Some(Value.Numa(y))) =>
  ...
  def input2state(n: List[Value.value]): Nat.nat => Option[Value.value] =
    ((a: Nat.nat) =>
      Map.map_of[Nat.nat, Value.value](Lista.enumerate[Value.value](Nat.zero_nata, n), a))

  def join_ir(i: List[Value.value], r: Nat.nat => Option[Value.value]): VName.vname => Option[Value.value] =
    ((a: VName.vname) => (a match {
      case VName.I(aa) => (input2state(i)).apply(aa)
...
object Cardinality {
  ...
  def card[A: card_UNIV: HOL.equal](x0: Set.set[A]): Nat.nat = x0 match {
    case Set.coset(xs) =>
      Nat.minus_nat(Phantom_Type.of_phantom[A, Nat.nat](card_UNIV[A]),
        Nat.Nata((xs.distinct).length))
    case Set.seta(xs) => Nat.Nata((xs.distinct).length)
  }

  def eq_set[A: card_UNIV: HOL.equal](x0: Set.set[A], x1: Set.set[A]): Boolean =
    (x0, x1) match {
      case (Set.coset(xs), Set.coset(ys)) =>
      ...
      case (Set.seta(xs), Set.seta(ys)) =>
        (Lista.list_all[A](((a: A) => ys contains a),
          xs)) && (Lista.list_all[A](((a: A) => xs contains a),
            ys))
      case (Set.seta(ys), Set.coset(xs)) =>
        {
          val n: Nat.nat = card[A](Set.top_set[A]);
          (if (Nat.equal_nata(n, Nat.zero_nata)) false
          else {
            val xsa: List[A] = xs.distinct
            val ysa: List[A] = ys.distinct;
            (Nat.equal_nata(Nat.plus_nata(Nat.Nata(xsa.length),
              Nat.Nata(ysa.length)),
              n)) && ((Lista.list_all[A](((x: A) =>
                !(ysa contains x)),
                xsa)) && (Lista.list_all[A](((y: A) => !(xsa contains y)),
                  ysa)))
          })
        }
      case (Set.coset(xs), Set.seta(ys)) =>
        {
          val n: Nat.nat = card[A](Set.top_set[A]);
          (if (Nat.equal_nata(n, Nat.zero_nata)) false
          else {
...
object Transition {
  ...
  def max_input(t: transition_ext[Unit]): Option[Nat.nat] =
    (if (Cardinality.eq_set[Nat.nat](enumerate_inputs(t), Set.bot_set[Nat.nat]))
      None else Some[Nat.nat](Lattices_Big.Max[Nat.nat](enumerate_inputs(t))))
...
object Transition_Ordering {
  ...
  def less_transition_ext[A: Orderings.linorder](t1: Transition.transition_ext[A],
    ...
    (if (Transition.Label[A](t1) == Transition.Label[A](t2))
      ...
      (if (Transition.Guard[A](t1) == Transition.Guard[A](t2))
      (if (Transition.Outputs[A](t1) == Transition.Outputs[A](t2))
      (if (Transition.Updates[A](t1) == Transition.Updates[A](t2))
...
object FSet {
  ...
  def ffUnion[A: HOL.equal](xa: fset[fset[A]]): fset[A] =
    Abs_fset[A](Complete_Lattices.Sup_set[A](Set.image[fset[A], Set.set[A]](((a: fset[A]) => fset[A](a)), fset[fset[A]](xa))))
  ...
  def sorted_list_of_fset[A: HOL.equal: Orderings.linorder](xa: fset[A]): List[A] =
  ...
  def size_fset[A: HOL.equal](f: fset[A]): Nat.nat =
    Finite_Set.card[A](fset[A](f))
...
object FSet_Utils {
  ...
  def fprod[A, B](xb: FSet.fset[A], xc: FSet.fset[B]): FSet.fset[(A, B)] =
    FSet.Abs_fset[(A, B)](Product_Type.product[A, B](FSet.fset[A](xb), FSet.fset[B](xc)))

  def fis_singleton[A: HOL.equal](s: FSet.fset[A]): Boolean =
    Nat.equal_nata(FSet.size_fset[A](s), Nat.one_nat)
...
object Store_Reuse_Subsumption {
  ...
  def stored_reused_aux_per_reg(ta: Transition.transition_ext[Unit],
    ...
    (if (Nat.equal_nata(r, Nat.zero_nata))
      ...
      Nat.Suc(Nat.minus_nat(r, Nat.one_nat)), p))
      Some[(Nat.nat, Nat.nat)]((Nat.Suc(Nat.minus_nat(r, Nat.one_nat)), p))
    else stored_reused_aux_per_reg(ta, t, Nat.minus_nat(r, Nat.one_nat),
      p)))
  ...
  def stored_reused_aux(ta: Transition.transition_ext[Unit],
    ...
    (if (Nat.equal_nata(p, Nat.zero_nata))
      ...
      Nat.Suc(Nat.minus_nat(p, Nat.one_nat))) match {
        case None =>
          stored_reused_aux(ta, t, r, Nat.minus_nat(p, Nat.one_nat))
  ...
  def stored_reused(ta: Transition.transition_ext[Unit],
    ...
    stored_reused_aux(ta, t, Inference.total_max_reg(e), Inference.max_output(e))
  ...
  def input_i_stored_in_reg(ta: Transition.transition_ext[Unit],
    ...
    (if (Nat.equal_nata(r, Nat.zero_nata))
      ...
      Nat.Suc(Nat.minus_nat(r, Nat.one_nat))))
      Some[(Nat.nat, Nat.nat)]((i, Nat.Suc(Nat.minus_nat(r, Nat.one_nat))))
    else input_i_stored_in_reg(ta, t, i, Nat.minus_nat(r, Nat.one_nat))))
  ...
  def input_stored_in_reg_aux(ta: Transition.transition_ext[Unit],
    ...
    else (input_i_stored_in_reg(ta, t, Nat.Suc(Nat.minus_nat(i, Nat.one_nat)),
      r) match {
        case None =>
          input_i_stored_in_reg(ta, t, Nat.minus_nat(i, Nat.one_nat), r)
  ...
  def input_stored_in_reg(ta: Transition.transition_ext[Unit],
    ...
    (input_stored_in_reg_aux(ta, t, Inference.total_max_reg(e),
      Store_Reuse.total_max_input(e)) match {
        case None => None
        case Some((i, r)) =>
    ...
    (stored_reused(ta, t, ea) match {
      ...
      case Some((p, r)) =>
        (if (Store_Reuse.is_generalised_output_of(ta, t, p, r))
          ((Transition.Outputs[Unit](t))(Code_Numeral.integer_of_nat(r).toInt) match {
          case AExp.L(v) =>
...
object Store_Reuse {
  ...
  def i_possible_steps(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    s: Nat.nat, r: Nat.nat => Option[Value.value], l: String,
    i: List[Value.value]): FSet.fset[(Nat.nat, (Nat.nat, Transition.transition_ext[Unit]))] =
  ...
  def i_step(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    s: Nat.nat, r: Nat.nat => Option[Value.value], l: String,
    i: List[Value.value]): Option[(Transition.transition_ext[Unit], (Nat.nat, (Nat.nat, Nat.nat => Option[Value.value])))] =
    {
      val possibilities: FSet.fset[(Nat.nat, (Nat.nat, Transition.transition_ext[Unit]))] = i_possible_steps(e, s, r, l, i);
      (if (FSet_Utils.fis_singleton[(Nat.nat, (Nat.nat, Transition.transition_ext[Unit]))](possibilities)) {
        val (u, (sa, t)): (Nat.nat, (Nat.nat, Transition.transition_ext[Unit])) = FSet.fthe_elem[(Nat.nat, (Nat.nat, Transition.transition_ext[Unit]))](possibilities);
        Some[(Transition.transition_ext[Unit], (Nat.nat, (Nat.nat, Nat.nat => Option[Value.value])))]((t,
  ...
  def modify(matches: List[(((Transition.transition_ext[Unit], Nat.nat), (ioTag, Nat.nat)), ((Transition.transition_ext[Unit], Nat.nat), (ioTag, Nat.nat)))],
    ...
    old: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      ...
      val newReg: Nat.nat = (Inference.max_reg(old) match {
        case None => Nat.one_nat
        case Some(r) => Nat.plus_nata(r, Nat.one_nat)
      })
  ...
  def indices(e: List[(String, (List[Value.value], List[Value.value]))]): FSet.fset[(Nat.nat, (ioTag, Nat.nat))] =
    Lista.foldl[FSet.fset[(Nat.nat, (ioTag, Nat.nat))], (Nat.nat, (String, (List[Value.value], List[Value.value])))](((a: FSet.fset[(Nat.nat, (ioTag, Nat.nat))]) =>
  ...
  def walk_up_to(n: Nat.nat,
    e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    s: Nat.nat, r: Nat.nat => Option[Value.value],
    x4: List[(String, (List[Value.value], List[Value.value]))]): (Transition.transition_ext[Unit], Nat.nat) =
    (n, e, s, r, x4) match {
      case (n, e, s, r, h :: t) =>
        {
          val (Some((transition, (sa, (uid, updated))))): Option[(Transition.transition_ext[Unit], (Nat.nat, (Nat.nat, Nat.nat => Option[Value.value])))] = i_step(e, s, r,
            Product_Type.fst[String, (List[Value.value], List[Value.value])](h),
            Product_Type.fst[List[Value.value], List[Value.value]](Product_Type.snd[String, (List[Value.value], List[Value.value])](h)));
    ...
    FSet.ffilter[((Nat.nat, (ioTag, Nat.nat)), (Nat.nat, (ioTag, Nat.nat)))](((a: ((Nat.nat, (ioTag, Nat.nat)), (Nat.nat, (ioTag, Nat.nat)))) =>
      {
        ...
        (Value.equal_valuea(lookup(aa, e),
          lookup(b, e))) && ((Nat.less_eq_nat(Product_Type.fst[Nat.nat, (ioTag, Nat.nat)](aa),
            Product_Type.fst[Nat.nat, (ioTag, Nat.nat)](b))) && (!(Product_Type.equal_proda[Nat.nat, (ioTag, Nat.nat)](aa, b))))
      }),
          ...
          ((ab: (Nat.nat, (ioTag, Nat.nat))) =>
            {
              ...
              ((walk_up_to(e1, e, Nat.zero_nata, ((_: Nat.nat) => None), t), (io1, inx1)),
                (walk_up_to(e2, e, Nat.zero_nata, ((_: Nat.nat) => None), t),
                  (io2, inx2)))
            })
        })(b)
      }),
        ...
        (old: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) =>
          (np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]) =>
            (modify(find_intertrace_matches(l, old), t1, t2, newa) match {
              case None => None
              case Some(newEFSM) =>
                Inference.resolve_nondeterminism(FSet.sorted_list_of_fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))](np(newEFSM)),
                  old, newEFSM,
  ...
  def total_max_input(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Nat.nat =
...
object Ignore_Inputs {
  ...
  def statewise_drop_inputs(t1ID: Nat.nat, t2ID: Nat.nat, s: Nat.nat,
    ...
    np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]): Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      val t1: Transition.transition_ext[Unit] = Inference.get_by_id(newa, t1ID);
      Inference.get_by_id(newa, t2ID);
      (if ((FSet.fBall[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](Inference.outgoing_transitions(s,
        newa))).apply(((a: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
        {
          val (_, (t, _)): (Nat.nat, (Transition.transition_ext[Unit], Nat.nat)) =
            a;
          (if (Transition.Label[Unit](t) == Transition.Label[Unit](t1))
            (if (Nat.equal_nata(Transition.Arity[Unit](t),
            Transition.Arity[Unit](t1)))
            Transition.Outputs[Unit](t) == Transition.Outputs[Unit](t1) else true)
          else true)
        })))
        Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](Inference.replace(Inference.drop_transitions(newa,
        FSet.minus_fset[Nat.nat](FSet.fimage[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat)), Nat.nat](Fun.comp[(Transition.transition_ext[Unit], Nat.nat), Nat.nat, (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](((a: (Transition.transition_ext[Unit], Nat.nat)) =>
          Product_Type.snd[Transition.transition_ext[Unit], Nat.nat](a)),
          ((a: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
            Product_Type.snd[Nat.nat, (Transition.transition_ext[Unit], Nat.nat)](a))),
          Inference.outgoing_transitions(s, newa)),
          FSet.finsert[Nat.nat](t1ID, FSet.bot_fset[Nat.nat]))),
        t1ID, drop_guards(t1)))
      else None)
    }
  ...
  def transitionwise_drop_inputs(t1ID: Nat.nat, t2ID: Nat.nat, s: Nat.nat,
    ...
    np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]): Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      ...
      (if (Transition.Outputs[Unit](t1) == Transition.Outputs[Unit](t2))
...
object Inference {
  ...
  def tm(t: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])] =
    FSet.fimage[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](a)),
      t)

  def dest(uid: Nat.nat,
    t: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Nat.nat =
    Product_Type.snd[Nat.nat, Nat.nat](Product_Type.fst[(Nat.nat, Nat.nat), Transition.transition_ext[Unit]](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](FSet.fthe_elem[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](FSet.ffilter[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](((x: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      Nat.equal_nata(Product_Type.fst[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](x),
        uid)),
      t)))))
  ...
  def replace_transition(t: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    uid: Nat.nat, from: Nat.nat, dest: Nat.nat,
    ...
    FSet.sup_fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](FSet.ffilter[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](((x: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      (!(Product_Type.equal_proda[(Nat.nat, Nat.nat), Transition.transition_ext[Unit]](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](x),
        ((from, dest),
          orig)))) && (!(Product_Type.equal_proda[(Nat.nat, Nat.nat), Transition.transition_ext[Unit]](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](x),
        ((from, dest), newa))))),
      t),
      FSet.finsert[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]((uid,
        ((from, dest), newa)),
  ...
  def origin(uid: Nat.nat,
    t: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Nat.nat =
    Product_Type.fst[Nat.nat, Nat.nat](Product_Type.fst[(Nat.nat, Nat.nat), Transition.transition_ext[Unit]](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](FSet.fthe_elem[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](FSet.ffilter[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](((x: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      Nat.equal_nata(Product_Type.fst[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](x),
        uid)),
      t)))))
  ...
  def merge_transitions(oldEFSM: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    (if (directly_subsumes(oldEFSM, destMerge, origin(u_1, oldEFSM),
      ...
      Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](replace_transition(destMerge,
      ...
      Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](replace_transition(destMerge,
      u_1, origin(u_1, destMerge), dest(u_1, destMerge), t_2, t_1))
  ...
  def make_distinct_aux(x0: List[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    (x0, e) match {
      case (Nil, e) => e
      case (h :: t, e) =>
        (if (FSet.fmember[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](h),
          FSet.fimage[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
            Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](a)),
            e)))
  ...
  def resolve_nondeterminism(x0: List[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))],
    uu: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    newa: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
  ...
  def merge(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    (if (Nat.equal_nata(s_1, s_2)) None
      ...
      resolve_nondeterminism(FSet.sorted_list_of_fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))](np(ea)),
        e, ea, m, check, np)
    })
  ...
  def outgoing_transitions(n: Nat.nat,
    ...
    FSet.fimage[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      {
        val (uid, (x, ta)): (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])) = a;
  ...
  def score(t: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    rank: (FSet.fset[Transition.transition_ext[Unit]]) => (FSet.fset[Transition.transition_ext[Unit]]) => Nat.nat): FSet.fset[(Nat.nat, (Nat.nat, Nat.nat))] =
    {
      val states: FSet.fset[(Nat.nat, Nat.nat)] =
        ...
        FSet.fimage[(Nat.nat, Nat.nat), (Nat.nat, (Nat.nat, Nat.nat))](((a: (Nat.nat, Nat.nat)) =>
          {
            ...
            ((rank(FSet.fimage[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat)), Transition.transition_ext[Unit]](((aa: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
              {
                val (_, (ta, _)): (Nat.nat, (Transition.transition_ext[Unit], Nat.nat)) = aa;
                ta
              }),
              outgoing_transitions(s1, t))))(FSet.fimage[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat)), Transition.transition_ext[Unit]](((aa: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
              {
                val (_, (ta, _)): (Nat.nat, (Transition.transition_ext[Unit], Nat.nat)) = aa;
                ta
              }),
    ...
    def infer(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
      r: (FSet.fset[Transition.transition_ext[Unit]]) => (FSet.fset[Transition.transition_ext[Unit]]) => Nat.nat,
      m: Nat.nat => Nat.nat => Nat.nat => (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => ((FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]) => Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]],
      check: (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])]) => Boolean,
      ...
      (inference_step(e, (FSet.sorted_list_of_fset[(Nat.nat, (Nat.nat, Nat.nat))](score(e, r))).reverse,
        m, check, np) match {
          case None => e
          case Some(newa) => {
            if (FSet.less_fset[Nat.nat](S(newa), S(e))) {
              println(s"The new EFSM has ${TypeConversion.fset_to_list(S(newa)).distinct.length} states and the old one had ${TypeConversion.fset_to_list(S(e)).distinct.length}")
              val pw = new PrintWriter(new File(s"dotfiles/step_${step}.dot" ))
              pw.write(EFSM_Dot.iefsm2dot(newa))
              pw.close
              step = step + 1
              return infer(newa, r, m, check, np)
            }
            else {
                println("I'm out!")
                return e
  ...
  def satisfies_trace(x1: List[(String, (List[Value.value], List[Value.value]))],
    x2: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])],
    x3: Nat.nat, x4: Nat.nat => Option[Value.value]): Boolean =
  ...
  def satisfies(t: Set.set[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    Set.Ball[List[(String, (List[Value.value], List[Value.value]))]](t,
      ((ta: List[(String, (List[Value.value], List[Value.value]))]) =>
        satisfies_trace(ta, e, Nat.zero_nata, ((_: Nat.nat) => None))))
  ...
  def make_branch(e: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])],
    uu: Nat.nat, uv: Nat.nat => Option[Value.value],
    ...
    (e, uu, uv, x3) match {
      ...
      case (e, s, r, (label, (inputs, outputs)) :: t) =>
        (EFSM.step(e, s, r, label, inputs) match {
          case None =>
            make_branch(FSet.finsert[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](((s,
          ...
          case Some((_, (sa, (outputsa, updated)))) =>
            ...
            else make_branch(FSet.finsert[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](((s, Nat.plus_nata(maxS(e), Nat.one_nat)),
              Transition.transition_exta[Unit](label,
                Nat.Nata(inputs.length),
                make_guard(inputs, Nat.zero_nata),
                make_outputs(outputs), Nil, ())),
  ...
  def make_pta(x0: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    (x0, e) match {
      case (Nil, e) => e
      case (h :: t, e) =>
        make_pta(t, make_branch(e, Nat.zero_nata, ((_: Nat.nat) => None), h))
    }
  ...
  def learn(l: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]): FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])] =
    {
      val pta: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])] =
        make_pta(l, FSet.bot_fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])])
  ...
  def choice(ta: Transition.transition_ext[Unit],
    ...
    Dirties.satisfiable(Lista.foldr[GExp.gexp, GExp.gexp](((v: GExp.gexp) =>
      (va: GExp.gexp) => GExp.Nor(GExp.Nor(v, v), GExp.Nor(va, va))),
      Transition.Guard[Unit](ta) ++ Transition.Guard[Unit](t), GExp.Bc(true)))
  ...
  def get_by_id(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    Product_Type.snd[(Nat.nat, Nat.nat), Transition.transition_ext[Unit]](Product_Type.snd[Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](FSet.fthe_elem[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](FSet.ffilter[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      {
  ...
  def max_output(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): Nat.nat =
    FSet.fMax[Nat.nat](FSet.fimage[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), Nat.nat](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
      {
        val (_, (_, t)): (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])) = a;
        Nat.Nata((Transition.Outputs[Unit](t)).length)
      }),
      e))
    ...
    (if (Nat.less_nat(FSet.size_fset[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](nt),
    ...
    else FSet.ffUnion[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))](FSet.fimage[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat)), FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]](((x: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
      {
        ...
        FSet.fimage[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat)), (Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))](((y: (Nat.nat, (Transition.transition_ext[Unit], Nat.nat))) =>
          ...
          FSet.minus_fset[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](nt, FSet.finsert[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat))](x, FSet.bot_fset[(Nat.nat, (Transition.transition_ext[Unit], Nat.nat))])))
      }),
      nt)))
          ...
          ((ad: (Transition.transition_ext[Unit], Nat.nat)) =>
            {
              ...
              (Transition.Label[Unit](ta) ==
                Transition.Label[Unit](taa)) && ((Nat.equal_nata(Transition.Arity[Unit](ta),
                  Transition.Arity[Unit](taa))) && (choice(ta, taa)))
            })
        })(b)
      }),
                ...
                ((ae: (Transition.transition_ext[Unit], Nat.nat)) =>
                  {
                    ...
                    (Transition.Label[Unit](ta) ==
                      Transition.Label[Unit](taa)) && ((Nat.equal_nata(Transition.Arity[Unit](ta),
                        Transition.Arity[Unit](taa))) && ((choice(ta,
                          taa)) || ((Transition.Outputs[Unit](ta) ==
...
object Code_Generation {
  ...
  def step(e: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])],
    s: Nat.nat, r: Nat.nat => Option[Value.value], l: String,
    i: List[Value.value]): Option[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))] =
    {
      val possibilities: FSet.fset[(Nat.nat, Transition.transition_ext[Unit])] =
        EFSM.possible_steps(e, s, r, l, i);
      (if (FSet.equal_fset[(Nat.nat, Transition.transition_ext[Unit])](possibilities,
        FSet.bot_fset[(Nat.nat, Transition.transition_ext[Unit])]))
        None
      else (Dirties.randomMember[(Nat.nat, Transition.transition_ext[Unit])](possibilities) match {
        case None => None
        case Some((sa, t)) =>
          {
            val outputs: List[Option[Value.value]] =
              EFSM.apply_outputs(Transition.Outputs[Unit](t),
                AExp.join_ir(i, r));
            Some[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))]((t,
              (sa, (outputs,
  ...
  def eq_i_i[A: HOL.equal](xa: A, xb: A): Predicate.pred[Unit] =
    Predicate.bind[(A, A), Unit](Predicate.single[(A, A)]((xa, xb)),
      ((a: (A, A)) =>
        {
          val (x, xaa): (A, A) = a;
  ...
  def guardMatch_alt(uu: List[GExp.gexp], uv: List[GExp.gexp]): Boolean = (uu, uv) match {
    case ((GExp.Eq(AExp.V(VName.I(ia)), AExp.L(Value.Numa(na)))) :: Nil,
      (GExp.Eq(AExp.V(VName.I(i)), AExp.L(Value.Numa(n)))) :: Nil) => (Nat.equal_nata(ia, Nat.zero_nata)) && (Nat.equal_nata(i, Nat.zero_nata))
    case (Nil, uv) => false
  ...
  def is_generalisation_of(ta: Transition.transition_ext[Unit],
    ...
    (Transition.equal_transition_exta[Unit](ta,
      Store_Reuse.remove_guard_add_update(t, i,
        r))) && ((Nat.less_nat(i, Transition.Arity[Unit](t))) && ((!((Lista.map[(Nat.nat, AExp.aexp), Nat.nat](((a: (Nat.nat, AExp.aexp)) =>
  ...
  def directly_subsumes_cases(a: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
  ...
  def satisfies_trace_i_i_i_i(x: List[(String, (List[Value.value], List[Value.value]))],
    xa: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])],
    xb: Nat.nat, xc: Nat.nat => Option[Value.value]): Predicate.pred[Unit] =
    Predicate.sup_pred[Unit](Predicate.bind[(List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value]))), Unit](Predicate.single[(List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value])))]((x, (xa, (xb, xc)))),
      ((a: (List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value])))) =>
        (a match {
          case (Nil, (_, (_, _))) => Predicate.single[Unit](())
          case (_ :: _, _) => Predicate.bot_pred[Unit]
        }))),
      Predicate.bind[(List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value]))), Unit](Predicate.single[(List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value])))]((x, (xa, (xb, xc)))),
        ((a: (List[(String, (List[Value.value], List[Value.value]))], (FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])], (Nat.nat, Nat.nat => Option[Value.value])))) =>
          (a match {
            case (Nil, _) => Predicate.bot_pred[Unit]
            case ((l, (i, p)) :: ex, (e, (s, d))) =>
              Predicate.bind[Option[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))], Unit](eq_i_o[Option[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))]](EFSM.step(e, s, d, l, i)),
                ((aa: Option[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))]) =>
                  (aa match {
                    case None => Predicate.bot_pred[Unit]
                    case Some((_, (sa, (pa, da)))) =>
                      Predicate.bind[Unit, Unit](satisfies_trace_i_i_i_i(ex, e, sa, da),
                        ((ab: Unit) =>
                          {
                            val (): Unit = ab;
                            Predicate.bind[Unit, Unit](eq_i_i[List[Option[Value.value]]](pa,
                              Lista.map[Value.value, Option[Value.value]](((ac: Value.value) =>
                                Some[Value.value](ac)),
                                p)),
                              ((ac: Unit) => {
                                val (): Unit = ac;
...
object EFSM {
  ...
  def step(x: FSet.fset[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])],
    xa: Nat.nat, xb: Nat.nat => Option[Value.value], xc: String,
    xd: List[Value.value]): Option[(Transition.transition_ext[Unit], (Nat.nat, (List[Option[Value.value]], Nat.nat => Option[Value.value])))] =
    Code_Generation.step(x, xa, xb, xc, xd)

  def apply_guards(g: List[GExp.gexp], s: VName.vname => Option[Value.value]): Boolean =
    Lista.list_all[Trilean.trilean](((ga: Trilean.trilean) =>
      Trilean.equal_trilean(ga, Trilean.truea())),
      Lista.map[GExp.gexp, Trilean.trilean](((ga: GExp.gexp) => GExp.gval(ga, s)), g))
  ...
  def apply_updates(x0: List[(Nat.nat, AExp.aexp)],
    ...
    newa: Nat.nat => Option[Value.value]): Nat.nat => Option[Value.value] =
    (x0, uu, newa) match {
      case (Nil, uu, newa) => newa
      case (h :: t, old, newa) =>
        ((x: Nat.nat) =>
          (if (Nat.equal_nata(x, Product_Type.fst[Nat.nat, AExp.aexp](h)))
            AExp.aval(Product_Type.snd[Nat.nat, AExp.aexp](h), old)
...
object FinFun {
  ...
  final case class finfun_update_code[A, B](a: finfun[A, B], b: A, c: B) extends finfun[A, B]

  def finfun_update[A: HOL.equal, B: HOL.equal](x0: finfun[A, B], a: A, b: B): finfun[A, B] =
    (x0, a, b) match {
...
object EFSM_Dot {
  ...
  def updates2dot_aux(x0: List[(Nat.nat, AExp.aexp)]): List[String] = x0 match {
    case Nil => Nil
    case h :: t =>
      (vname2dot(VName.R(Product_Type.fst[Nat.nat, AExp.aexp](h))) + " := " +
        aexp2dot(Product_Type.snd[Nat.nat, AExp.aexp](h))) :: (updates2dot_aux(t))
  }
  ...
  def iefsm2dot(e: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]): String =
    "digraph EFSM{" + "\u000A" + "  graph [rankdir=" + "\"" + "LR" + "\"" +
      ...
      "];" +
      "\u000A" +
      join(FSet.sorted_list_of_fset[String](FSet.fimage[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), String](((a: (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
        {
          val (uid, aa): (Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])) = a
          val (ab, b): ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]) = aa;
          ({
...
object Groups_List {
  ...
  def sum_list[A: Groups.monoid_add](xs: List[A]): A =
    Lista.foldr[A, A](((a: A) => (b: A) => Groups.plus[A](a, b)), xs,
      Groups.zero[A])
...
object Same_Register {
  ...
  def updates(x0: List[(Nat.nat, AExp.aexp)]): Option[VName.vname] = x0 match {
    case (n, a) :: Nil => Some[VName.vname](VName.R(n))
    case Nil => None
    case v :: (vb :: vc) => None
  }
...
object Type_Inference {
  ...
  def fun_of[A: HOL.equal, B: HOL.equal](x0: List[(A, B)], b: B): FinFun.finfun[A, B] =
    (x0, b) match {
  ...
  def type_of(x0: AExp.aexp, uv: FinFun.finfun[VName.vname, typea]): typea =
    (x0, uv) match {
      case (AExp.L(Value.Numa(uu)), uv) => NUM()
      case (AExp.L(Value.Str(uw)), ux) => STRING()
      case (AExp.V(v), types) => FinFun.finfun_apply[VName.vname, typea](types, v)
      case (AExp.Plus(uy, uz), va) => NUM()
      case (AExp.Minus(vb, vc), vd) => NUM()
    }

  def assign_all(t: typea, l: List[VName.vname]): List[(VName.vname, typea)] =
    (Lista.map[VName.vname, (VName.vname, typea)](((v: VName.vname) => (v, t)), l)).distinct

  def type_check_var(v: VName.vname, l: List[(VName.vname, typea)]): Boolean =
    Nat.less_eq_nat(Nat.one_nat,
      Finite_Set.card[typea](Set.seta[typea](Lista.map[(VName.vname, typea), typea](((a: (VName.vname, typea)) =>
        {
          val (_, t): (VName.vname, typea) = a;
          t
        }),
        Lista.filter[(VName.vname, typea)](((a: (VName.vname, typea)) =>
          {
            val (va, _): (VName.vname, typea) = a;
            VName.equal_vnamea(va, v)
          }),
          l)))))

  def type_check(l: List[(VName.vname, typea)]): Boolean =
    Lista.list_all[(VName.vname, typea)](((x: (VName.vname, typea)) =>
      type_check_var(Product_Type.fst[VName.vname, typea](x),
        l)),
      l)
  ...
  def assign_group_types(x0: List[List[VName.vname]],
    types: List[(VName.vname, typea)]): List[(VName.vname, typea)] =
    (x0, types) match {
  ...
  def infer_types_aux(x0: GExp.gexp): (List[(VName.vname, typea)], List[(VName.vname, VName.vname)]) =
    x0 match {
      ...
      case GExp.Nor(g1, g2) =>
        {
          val (t1, g1a): (List[(VName.vname, typea)], List[(VName.vname, VName.vname)]) = infer_types_aux(g1)
          val (t2, g2a): (List[(VName.vname, typea)], List[(VName.vname, VName.vname)]) = infer_types_aux(g2);
  ...
  def collapse_group(x0: (VName.vname, VName.vname), x1: List[List[VName.vname]]): List[List[VName.vname]] =
    (x0, x1) match {
  ...
  def collapse_groups(x0: List[(VName.vname, VName.vname)],
    g: List[List[VName.vname]]): List[List[VName.vname]] =
    (x0, g) match {
      case (Nil, g) => g
  ...
  def infer_types(g: GExp.gexp): Option[FinFun.finfun[VName.vname, typea]] =
    {
      val (types, groups): (List[(VName.vname, typea)], List[(VName.vname, VName.vname)]) = infer_types_aux(g)
      val type_lst: List[(VName.vname, typea)] =
        assign_group_types(collapse_groups(groups, Nil), types);
      (if (type_check(type_lst))
        Some[FinFun.finfun[VName.vname, typea]](fun_of[VName.vname, typea](type_lst, UNBOUND()))
      else None)
    }
  ...
  def aexp_type_check(a1: AExp.aexp, a2: AExp.aexp,
    t: FinFun.finfun[VName.vname, typea]): Boolean =
      ...
      (if ((guardMatch[Unit, Unit](t1, t2)) && (outputMatch[Unit, Unit](t1, t2))) {
        ...
        val newT1: Transition.transition_ext[Unit] =
          Transition.transition_exta[Unit](Transition.Label[Unit](t1),
            ...
            (AExp.Plus(AExp.V(newReg), AExp.V(VName.I(Nat.zero_nata)))) :: Nil,
            (r, AExp.Plus(AExp.V(newReg),
              AExp.V(VName.I(Nat.zero_nata)))) :: (Transition.Updates[Unit](t1)),
            ())
        val newT2: Transition.transition_ext[Unit] =
          Transition.transition_exta[Unit](Transition.Label[Unit](t2),
            ...
            (AExp.Plus(AExp.V(newReg), AExp.V(VName.I(Nat.zero_nata)))) :: Nil,
            (r, AExp.Plus(AExp.V(newReg),
              AExp.V(VName.I(Nat.zero_nata)))) :: (Transition.Updates[Unit](t2)),
            ())
...
object SelectionStrategies {
  ...
  def bool2nat(x0: Boolean): Nat.nat = x0 match {
    case true => Nat.one_nat
    case false => Nat.zero_nata
  }

  def naive_score(t1: FSet.fset[Transition.transition_ext[Unit]],
    t2: FSet.fset[Transition.transition_ext[Unit]]): Nat.nat =
    FSet.size_fset[(Transition.transition_ext[Unit], Transition.transition_ext[Unit])](FSet.ffilter[(Transition.transition_ext[Unit], Transition.transition_ext[Unit])](((a: (Transition.transition_ext[Unit], Transition.transition_ext[Unit])) =>
      {
        val (x, y): (Transition.transition_ext[Unit], Transition.transition_ext[Unit]) = a;
        (Transition.Label[Unit](x) ==
          Transition.Label[Unit](y)) && (Nat.equal_nata(Transition.Arity[Unit](x),
            Transition.Arity[Unit](y)))
      }),
      FSet_Utils.fprod[Transition.transition_ext[Unit], Transition.transition_ext[Unit]](t1, t2)))

  def naive_score_one_final_state(t1: FSet.fset[Transition.transition_ext[Unit]],
    t2: FSet.fset[Transition.transition_ext[Unit]]): Nat.nat =
    (if ((FSet.equal_fset[Transition.transition_ext[Unit]](t1,
      FSet.bot_fset[Transition.transition_ext[Unit]])) && (FSet.equal_fset[Transition.transition_ext[Unit]](t2,
        FSet.bot_fset[Transition.transition_ext[Unit]])))
      Nat.one_nat
    else FSet.size_fset[(Transition.transition_ext[Unit], Transition.transition_ext[Unit])](FSet.ffilter[(Transition.transition_ext[Unit], Transition.transition_ext[Unit])](((a: (Transition.transition_ext[Unit], Transition.transition_ext[Unit])) =>
      {
        val (x, y): (Transition.transition_ext[Unit], Transition.transition_ext[Unit]) = a;
        (Transition.Label[Unit](x) ==
          Transition.Label[Unit](y)) && (Nat.equal_nata(Transition.Arity[Unit](x),
  ...
  def naive_score_rank_one_final_state(t1: FSet.fset[Transition.transition_ext[Unit]],
    t2: FSet.fset[Transition.transition_ext[Unit]]): Nat.nat =
    (if ((FSet.equal_fset[Transition.transition_ext[Unit]](t1,
      FSet.bot_fset[Transition.transition_ext[Unit]])) && (FSet.equal_fset[Transition.transition_ext[Unit]](t2,
        FSet.bot_fset[Transition.transition_ext[Unit]])))
      Nat.one_nat
    else Groups_Big.sum[Nat.nat, Nat.nat](((x: Nat.nat) => x),
      FSet.fset[Nat.nat](FSet.fimage[(Transition.transition_ext[Unit], Transition.transition_ext[Unit]), Nat.nat](((a: (Transition.transition_ext[Unit], Transition.transition_ext[Unit])) =>
        {
          val (x, y): (Transition.transition_ext[Unit], Transition.transition_ext[Unit]) =
            a;
          Nat.plus_nata(Nat.plus_nata(bool2nat(Transition.Label[Unit](x) ==
            ...
            bool2nat(Nat.equal_nata(Transition.Arity[Unit](x),
              Transition.Arity[Unit](y)))),
            bool2nat(Transition.Outputs[Unit](x) ==
