...
def lob(t1: Transition.transition_ext[Unit],
  ...
  (if ((Transition.Guards[Unit](t1)).isEmpty) t1
           ...
           else Transition.Guards_update[Unit](((_:
           List[GExp.gexp[VName.vname]])
          =>
   ...
   (! (Lista.equal_lista[Nat.nat](id1, id2))) && (Transition.identical_except_guard(t1,
     t2))))
                          ...
                          aa)
                                  ...
                                  state_pairs)):
        (FSet.fset[(Nat.nat, (Nat.nat, Boolean))])
...
def ID[A](x0: Transition.transition_ext[iTransition_ext[A]]): List[Nat.nat] = x0
  match {
...
def Origin[A](x0: Transition.transition_ext[iTransition_ext[A]]): Nat.nat = x0
  match {
  case Transition.transition_exta(label, arity, guards, outputs, updates,
                                   iTransition_exta(id, origin, destination,
             more))
...
def guards2dot(x0: List[GExp.gexp[VName.vname]]): String = x0 match {
  ...
  case (v::va) =>
    "&#91;" +
      (Lista.map[GExp.gexp[VName.vname],
                  String](Fun.comp[String, String,
                                    GExp.gexp[VName.vname]](((a: String) =>
...
def process_success(gen: (FSet.fset[(List[Nat.nat],
                                      ((Nat.nat, Nat.nat),
Transition.transition_ext[Unit]))],
                           (Map[((String,
                                  (List[value_type],
                     ...
                     gp: ((String, (List[value_type], List[value_type])),
                           List[(List[Nat.nat],
                                  Transition.transition_ext[Unit])]),
                     t: List[((String, (List[value_type], List[value_type])),
                               List[(List[Nat.nat],
                                      Transition.transition_ext[Unit])])],
                     update_groups:
...
Transition.transition_ext[Unit])](take_maximum_updates(different)),
    ...
    val new_outputs =
      ((tid: List[Nat.nat]) => (ta: Transition.transition_ext[Unit]) =>
        ...
        ((List[Nat.nat]) =>
          (Transition.transition_ext[Unit]) => List[AExp.aexp[VName.vname]])
    val new_updates =
      ((tid: List[Nat.nat]) => (ta: Transition.transition_ext[Unit]) =>
        (funs(structure(tid)) match {
                    ...
                    val (i, (_, _)) =
                      a: ((List[Nat.nat],
                           ((Nat.nat, Nat.nat),
                             Transition.transition_ext[Unit])));
                    Product_Type.equal_proda[String,
      (List[value_type], List[value_type])](structure(i), structure(rep_id))
                  }),
                 e)))):
             (((String, (List[value_type], List[value_type])),
               List[(List[Nat.nat], Transition.transition_ext[Unit])]));
         (generalise_and_update(transition_repeats, transition_repeats, bad,
            ...
            case Success((ea, (output_mema,
              ...
              => process_success((ea, (output_mema,
(update_mema, (output_funs, bada)))),
...
def ehw_distinguish(pta: FSet.fset[(List[Nat.nat],
                                     ((Nat.nat, Nat.nat),
                                       Transition.transition_ext[Unit]))],
                     log: List[List[(String,
                                      (List[Value.value], List[Value.value]))]],
...
Transition.transition_ext[Unit]))],
  ...
  =
  {
    ...
    val t2 =
      Inference.get_by_ids(destMerge, t2ID): (Transition.transition_ext[Unit])
    val (g1, g2) =
      collect_training_sets(log, pta, t1ID, t2ID, Nil, Nil):
        ((List[(List[Value.value], Map[Nat.nat, Option[Value.value]])],
          List[(List[Value.value], Map[Nat.nat, Option[Value.value]])]));
    (Dirties.findDistinguishingGuards(g1, g2, t1ID, t1, t2ID, t2) match {
       case None => None
  ...
  case (((label, (inputs, outputs))::t), e1, s1, r1, e2, s2, r2) =>
    {
      val possible_steps_e1 =
        FSet.ffilter[(List[Nat.nat],
                 ...
                 outputs))
                     ...
                     Inference.i_possible_steps(e1, s1, r1, label, inputs)):
          (FSet.fset[(List[Nat.nat],
                       (Nat.nat, Transition.transition_ext[Unit]))])
      val (u1, (dest1, t1)) =
        FSet.fthe_elem[(List[Nat.nat],
                         (Nat.nat,
                           Transition.transition_ext[Unit]))](possible_steps_e1):
          ((List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit])))
      val possible_steps_e2 =
        FSet.ffilter[(List[Nat.nat],
                     ...
                     Inference.i_possible_steps(e2, s2, r2, label, inputs)):
          (FSet.fset[(List[Nat.nat],
                       (Nat.nat, Transition.transition_ext[Unit]))])
      val (_, (dest2, t2)) =
        FSet.fthe_elem[(List[Nat.nat],
                         (Nat.nat,
                           Transition.transition_ext[Unit]))](possible_steps_e2):
          ((List[Nat.nat], (Nat.nat, Transition.transition_ext[Unit])))
      val r1a =
        (Transition.apply_updates(Transition.Updates[Unit](t1),
                                   AExp.join_ir(inputs, r1))).apply(r1):
          Map[Nat.nat, Option[Value.value]]
      val a =
        (Transition.apply_updates(Transition.Updates[Unit](t2),
                                   AExp.join_ir(inputs, r2))).apply(r2):
          Map[Nat.nat, Option[Value.value]];
