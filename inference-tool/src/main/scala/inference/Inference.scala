    ...
    Lista.foldr[(A, B), A => Option[B]](((a: (A, B)) =>
      {
        val (k, v): (A, B) = a;
        ((ps: A => Option[B]) =>
          Fun.fun_upd[A, Option[B]](ps, k, Some[B](v)))
      }),
      l, ((_: A) => None))
...
object AExp {
  ...
  def input2state(n: List[Value.value]): Nat.nat => Option[Value.value] =
    Map.map_of[Nat.nat, Value.value](Lista.enumerate[Value.value](Nat.zero_nata, n))
...
object FSet {
  ...
  def sorted_list_of_fset[A: HOL.equal: Orderings.linorder](x0: fset[A]): List[A] =
    x0 match {
      case fset_of_list(as) => Lista.sort_key[A, A](((x: A) => x), as.distinct)
    }

  def less_eq_fset[A: HOL.equal](x0: fset[A], a: fset[A]): Boolean = (x0, a) match {
  ...
  def equal_fset[A: HOL.equal](x: fset[A], xa1: fset[A]): Boolean = (x, xa1) match {
    case (x, fset_of_list(y)) =>
      (Nat.equal_nata(size_fset[A](x),
        Nat.Nata((y.distinct).length))) && (fBall[A](x)).apply(((a: A) =>
          y contains a))
  }
  ...
  def fprod[A, B](x0: FSet.fset[A], x1: FSet.fset[B]): FSet.fset[(A, B)] =
    (x0, x1) match {
      case (FSet.fset_of_list(xs), FSet.fset_of_list(ys)) =>
        FSet.fset_of_list[(A, B)](Lista.maps[A, (A, B)](((x: A) => Lista.map[B, (A, B)](((a: B) => (x, a)), ys)), xs))
    }
...
object Store_Reuse_Subsumption {
  ...
  def stored_reused(ta: Transition.transition_ext[Unit],
    ...
    stored_reused_aux(ta, t, Inference.total_max_reg(e),
...
object Store_Reuse {
  ...
  def walk_up_to(n: Nat.nat,
    ...
    (n, e, s, r, x4) match {
      case (n, e, s, r, h :: t) =>
        {
          val (Some((transition, (sa, (uid, updated))))): Option[(Transition.transition_ext[Unit], (Nat.nat, (Nat.nat, Nat.nat => Option[Value.value])))] = i_step(e, s, r, h._1, (h._2)._1);
...
object Ignore_Inputs {
  ...
  def transitionwise_drop_inputs(t1ID: Nat.nat, t2ID: Nat.nat, s: Nat.nat,
    ...
    np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]): Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      ...
      println(PrettyPrinter.transitionToString(t1))
      println(PrettyPrinter.transitionToString(t2))
      (if (Lista.equal_lista[AExp.aexp](Transition.Outputs[Unit](t1), Transition.Outputs[Unit](t2))) {
        val n = Inference.replace(Inference.drop_transition(newa, t2ID),t1ID, drop_guards(t1))
        PrettyPrinter.iEFSM2dot(n, Nat.Nata(20));
        Some(Inference.replace(Inference.drop_transition(newa, t2ID),t1ID, drop_guards(t1)))
      }
      else {
        println("They should be equal...")
        None
      })
    }
...
object Inference {
  ...
  def make_guard(x0: List[Value.value], uu: Nat.nat): List[GExp.gexp] = (x0, uu) match {
    ...
    case (h :: t, n) =>
      (GExp.Eq(AExp.V(VName.I(n)),
        AExp.L(h))) :: (make_guard(t, Nat.plus_nata(n, Nat.Nata((1)))))
  }
      ...
      case (e, s, r, (label, (inputs, outputs)) :: t) =>
        (EFSM.step(e, s, r, label, inputs) match {
          ...
          case Some((_, (sa, (outputsa, updated)))) =>
            ...
            else make_branch(FSet.finsert[((Nat.nat, Nat.nat), Transition.transition_ext[Unit])](((s,
              Nat.plus_nata(maxS(e), Nat.Nata((1)))),
              Transition.transition_exta[Unit](label,
                Nat.Nata(inputs.length),
                make_guard(inputs, Nat.zero_nata),
  ...
  def merge_transitions(oldEFSM: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    np: (FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], Nat.nat), (Transition.transition_ext[Unit], Nat.nat))))]): Option[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] = {
      println(s"  Trying to merge transitions\n    ${PrettyPrinter.transitionToString(t_1)}\n    ${PrettyPrinter.transitionToString(t_2)}")
      step = step + 1
      if (directly_subsumes(oldEFSM, destMerge, origin(u_1, oldEFSM), origin(u_1, destMerge), t_2, t_1)) {
        println("    Directly subsumes!")
        PrettyPrinter.iEFSM2dot(drop_transition(destMerge, u_1), Nat.Nata(step));
        Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](drop_transition(destMerge, u_1))
      }
      else if (directly_subsumes(oldEFSM, destMerge, origin(u_2, oldEFSM), origin(u_2, destMerge), t_1, t_2)) {
        println("    Directly subsumes!")
        PrettyPrinter.iEFSM2dot(drop_transition(destMerge, u_2), Nat.Nata(step));
        Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](drop_transition(destMerge, u_2))
      }
      else {
        println("    Need to use the heuristic")
        (((((modifier(u_1))(u_2))(origin(u_1, destMerge)))(destMerge))(oldEFSM))(np)
      }
    }
  ...
  def choice(ta: Transition.transition_ext[Unit], t: Transition.transition_ext[Unit]): Boolean = {
    println(s"    Choice between\n      ${PrettyPrinter.transitionToString(ta)}\n      ${PrettyPrinter.transitionToString(t)}?\n    ${Code_Generation.choice_cases(ta, t)}")
    Code_Generation.choice_cases(ta, t)
  }
  ...
  def inference_step(uu: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    (uu, x1, uv, uw, ux) match {
      ...
      case (e, (uy, (s_1, s_2)) :: t, m, check, np) => {
        println(s"Trying to merge states ${s_1} and ${s_2}")
        (merge(e, s_1, s_2, m, check, np) match {
          case None => {
            println(s"  Failed to merge states ${s_1} and ${s_2}")
            inference_step(e, t, m, check, np)
          }
          case Some(a) => {
            println(s"  Successfully merged states ${s_1} and ${s_2}")
            PrettyPrinter.iEFSM2dot(a, Nat.Nata(19));
            Some[FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](a)
          }
        })}
    }
...
object Code_Generation {
  ...
  def mutex(uu: GExp.gexp, uv: GExp.gexp): Boolean = {
    (uu, uv) match {
    case (GExp.Eq(AExp.V(va), AExp.L(la)), GExp.Eq(AExp.V(v), AExp.L(l))) => {
      // println(s"      ${va} = ${v}? ${VName.equal_vnamea(va, v)}")
      if (VName.equal_vnamea(va, v)) {
        // println(s"      ${la} != ${l}? ${!(Value.equal_valuea(la, l))}")
        !(Value.equal_valuea(la, l))
      }
      else false
    }
  ...
  def choice_cases(t1: Transition.transition_ext[Unit],
    ...
    else (if (Lista.list_ex[(GExp.gexp, GExp.gexp)](((a: (GExp.gexp, GExp.gexp)) =>
      {
        val (aa, b): (GExp.gexp, GExp.gexp) = a;
        println(s"      Mutex ${PrettyPrinter.gexpToString(aa)} and ${PrettyPrinter.gexpToString(b)}? ${mutex(aa, b)}")

        mutex(aa, b)
      }),
      Lista.product[GExp.gexp, GExp.gexp](Transition.Guard[Unit](t1),
        Transition.Guard[Unit](t2))))
      false
  ...
  def directly_subsumes_cases(a: FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    ...
    if (Transition.equal_transition_exta[Unit](t1, t2)) {
      ...
      Transition.Outputs[Unit](t2))) && (Dirties.alwaysDifferentOutputsDirectSubsumption(a,
        b, sa, s, t1, t2))) {
          println("    Always different outputs")
          false
        }
    else (if (Store_Reuse_Subsumption.drop_guard_add_update_direct_subsumption(t1, t2, b, s)){
      println("    Store and reuse")
      true
    }
    else (if (Store_Reuse_Subsumption.generalise_output_direct_subsumption(t1, t2, b, a, sa, s)) {
      println("    generalise output")
      true
    }
    else (if (Transition.equal_transition_exta[Unit](t1, Ignore_Inputs.drop_guards(t2))) {
      println("    drop guards")
      true
    }
...
object EFSM {
  ...
  def apply_updates(x0: List[(Nat.nat, AExp.aexp)],
    ...
    (x0, uu, newa) match {
      ...
      case (h :: t, old, newa) =>
        ((x: Nat.nat) =>
          (if (Nat.equal_nata(x, h._1)) AExp.aval(h._2, old)
...
object Type_Inference {
  ...
  def fun_of[A: HOL.equal, B: HOL.equal](x0: List[(A, B)], b: B): FinFun.finfun[A, B] =
    (x0, b) match {
      ...
      case (h :: t, b) => FinFun.finfun_update[A, B](fun_of[A, B](t, b), h._1, h._2)
    }
  ...
  def collapse_group(x0: (VName.vname, VName.vname), x1: List[List[VName.vname]]): List[List[VName.vname]] =
    (x0, x1) match {
      case ((v1, v2), Nil) => ((v1 :: (v2 :: Nil)).distinct) :: Nil
      case ((v1, v2), h :: t) =>
