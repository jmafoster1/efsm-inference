  ...
  def distinguish(log: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    np: (FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], List[Nat.nat]), (Transition.transition_ext[Unit], List[Nat.nat]))))]): Option[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      ...
      (Dirties.findDistinguishingGuard(g1, g2) match {
        ...
        case Some((g1a, g2a)) =>
          {
            val newEFSM: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = Inference.replace_transitions(preDestMerge, List((t1ID, add_guard(t1, g1a)), (t2ID, add_guard(t2, g2a))));
            PrettyPrinter.iEFSM2dot(newEFSM, "newEFSM")
            Inference.resolve_nondeterminism(Nil,
              FSet.sorted_list_of_fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], List[Nat.nat]), (Transition.transition_ext[Unit], List[Nat.nat]))))](np(newEFSM)),
              old, newEFSM,
              ((a: List[Nat.nat]) => (b: List[Nat.nat]) => (c: Nat.nat) =>
              ...
              np) match {
                case None => None
                case Some(newEFSMResolved) => {
                  PrettyPrinter.iEFSM2dot(newEFSMResolved, "newEFSMResolved")
                  Some(newEFSMResolved)
                }
              }
