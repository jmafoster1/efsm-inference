  ...
  def resolve_nondeterminism(uu: List[(Nat.nat, Nat.nat)],
    ...
    (uu, x1, uv, newa, uw, check, np) match {
      case (uu, Nil, uv, newa, uw, check, np) => {
        if (deterministic(newa, np)) {
          if (check(tm(newa)))
            Some[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]](newa)
          else {
            println("Does not accept the original traces")
            None
          }
        }
        else {
          println("EFSM is still nondeterministic")
          None
        }
      }
...
object PTA_Generalisation {
  ...
  def derestrict(log: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    {
      val pta: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = Inference.make_pta(log, FSet.bot_fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))])
      val normalised: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = normalised_pta(log)

      PrettyPrinter.iEFSM2dot(normalised, "normalised")
      Log.root.info("The normalised pta is " +
        (if (Inference.nondeterministic(normalised, Inference.nondeterministic_pairs)) "non" else "") + "deterministic")
      ...
      val derestricted: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = FSet.fimage[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit])), (List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))](((a: (List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))) =>
        ...
        Log.root.info("The derestricted pta is " +
          (if (Inference.nondeterministic(derestricted, Inference.nondeterministic_pairs)) "non" else "") + "deterministic")
      ...
      println(nondeterministic_pairs)
      (Inference.resolve_nondeterminism(Nil, nondeterministic_pairs, pta,
        ...
        np) match {
          case None => {
            println("Could not resolve nondeterminsm!")
            pta
          }
...
object Distinguishing_Guards {
  ...
  def distinguish(log: List[List[(String, (List[Value.value], List[Value.value]))]],
    ...
    np: (FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => FSet.fset[(Nat.nat, ((Nat.nat, Nat.nat), ((Transition.transition_ext[Unit], List[Nat.nat]), (Transition.transition_ext[Unit], List[Nat.nat]))))]): Option[FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]] =
    {
      ...
      val t2: Transition.transition_ext[Unit] =
        Inference.get_by_ids(destMerge, t2ID)
      val uPTA: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))] = transfer_updates(destMerge,
