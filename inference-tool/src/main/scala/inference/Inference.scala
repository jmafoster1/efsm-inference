...
def infer_updates(e: FSet.fset[(List[Nat.nat],
                                 ((Nat.nat, Nat.nat),
                                   Transition.transition_ext[Unit]))],
                   l: List[List[(String,
                                  (List[Value.value], List[Value.value]))]],
...
def groupwise_infer_updates(l: List[List[(String,
   (List[Value.value], List[Value.value]))]],
                             e: FSet.fset[(List[Nat.nat],
    ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
...
def get_output(label: String, maxReg: Nat.nat, values: List[Value.value],
  ...
  (Lista.find[(AExp.aexp[VName.vname],
    ...
    fun_mem(label))
     match {
     case None => Dirties.getOutput(label, maxReg, values, bad, train)
     case Some((fun, types)) =>
       Some[(AExp.aexp[VName.vname], Map[VName.vname, String])]((fun, types))
   })

def output_and_update(uu: List[AExp.aexp[VName.vname]],
                       fun_mem:
                         Map[String, (List[(AExp.aexp[VName.vname],
     Map[VName.vname, String])])],
                       uv: Nat.nat, uw: Nat.nat,
                       e: FSet.fset[(List[Nat.nat],
                                      ((Nat.nat, Nat.nat),
Transition.transition_ext[Unit]))],
                       ux: List[List[(String,
                                       (List[Value.value],
 List[Value.value]))]],
                       uy: List[List[(List[Nat.nat],
                                       Transition.transition_ext[Unit])]],
                       uz: List[(List[Nat.nat],
                                  Transition.transition_ext[Unit])],
...
g)))
 ...
 Transition.transition_ext[Unit])]]
   ...
   gps)
    ...
    ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]
                         = groupwise_infer_updates(log, ea, possible_gps,
...
def finfun_filter[A : HOL.equal : Orderings.linorder,
                   B](fa: Map[A, B], f: A => Boolean):
      Map[A, B]
  =
  {
    ...
    Lista.fold[A, Map[A, B]](((k: A) => (acc: Map[A, B]) =>
                               (acc + ((k -> (fa(k)))))),
                              keys,
                              scala.collection.immutable.Map().withDefaultValue(Dirties.defaultValue[A,
                      B](fa)))
  }
          ...
          (Map[String, (List[(AExp.aexp[VName.vname],
            ...
            List[(Transition.transition_ext[Unit],
                   List[Option[(AExp.aexp[VName.vname],
                                 Map[VName.vname, String])]])])))
  =
  (uu, e, x2, update_groups, structure, funs, to_derestrict, closed, fun_mem)
  match {
  case (uu, e, Nil, update_groups, structure, funs, to_derestrict, closed,
         fun_mem)
    => (e, (to_derestrict, (fun_mem, closed)))
    ...
    => {
         ...
         val (ea, fun_mema):
               (FSet.fset[(List[Nat.nat],
                 ...
                 Map[String, (List[(AExp.aexp[VName.vname],
                                     Map[VName.vname, String])])])
           = generalise_and_update(log, e, gp, update_groups, fun_mem)
         val different:
               FSet.fset[(List[Nat.nat],
                           ((Nat.nat, Nat.nat),
                             Transition.transition_ext[Unit]))]
            ...
            (List[List[Nat.nat]],
              (Map[String, (List[(AExp.aexp[VName.vname],
                ...
                List[(Transition.transition_ext[Unit],
                       List[Option[(AExp.aexp[VName.vname],
                                     Map[VName.vname, String])]])])))
      = groupwise_generalise_and_update(log, pta, groups, groups,
