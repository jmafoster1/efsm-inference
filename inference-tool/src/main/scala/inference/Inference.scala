...
object Inference {
  ...
  def step_score(x0: List[(List[Nat.nat], List[Nat.nat])],
    uu: FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))],
    uv: (List[Nat.nat]) => (List[Nat.nat]) => (FSet.fset[(List[Nat.nat], ((Nat.nat, Nat.nat), Transition.transition_ext[Unit]))]) => Nat.nat): Nat.nat =
    (x0, uu, uv) match {
...
object Weak_Subsumption {
  ...
  def maxBy[A, B: Orderings.ord](f: A => B, a: A, b: A): A =
