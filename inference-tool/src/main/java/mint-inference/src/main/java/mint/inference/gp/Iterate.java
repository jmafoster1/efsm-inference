...
public class Iterate extends AbstractIterator {
	...
	public Iterate(List<Chromosome> population, float crossOver, float mutation, Generator g, int maxD, Random r,
	...
	public void mutate() {
		for (Chromosome c : population) {
			Node<?> aNode = (Node<?>) c;
			aNode = aNode.simp();
			double toMutate = rand.nextDouble();

			if (isUnviable(aNode) || toMutate < mutation)
				continue;
			offSpring.add(aNode);
			aNode.mutate(gen, rand.nextInt(maxDepth - aNode.depth()));
		}
	}

	private boolean isUnviable(Node<?> n) {
		return (n instanceof RootDouble || n instanceof RootBoolean || n instanceof RootInteger
				|| n instanceof RootListNonTerminal || n.depth() >= maxDepth);
	}
