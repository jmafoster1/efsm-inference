...
public class SymbolicRegressionEngine {
	...
	private static final int MAX_INITIAL_TREE_DEPTH = 4;
	...
	private Population<GpChromosome> createPopulation(
			Context context,
			Fitness<GpChromosome, Double> fitnessFunction,
			int populationSize
			) {
		Population<GpChromosome> population = new Population<GpChromosome>();
		for (int i = 0; i < populationSize; i++) {
			GpChromosome chromosome = new GpChromosome(context, fitnessFunction,
	...
	public SymbolicRegressionEngine(
			int[] values,
			ExpressionFitness expressionFitness,
			Collection<String> variables,
			List<? extends Function> baseFunctions
			) {
		this.context = new Context(values, baseFunctions, variables);
		this.expressionFitness = expressionFitness;
		Fitness<GpChromosome, Double> fitnessFunction = new SymbolicRegressionFitness(this.expressionFitness);
		Population<GpChromosome> population = this.createPopulation(this.context, fitnessFunction,
...
//		Population<GpChromosome> population = new Population<GpChromosome>();
		for (Expression e: extras) {
			GpChromosome chromosome = new GpChromosome(context, fitnessFunction,e);
			population.addChromosome(chromosome);
		}
		this.environment = new GeneticAlgorithm<GpChromosome, Double>(population, fitnessFunction);
		this.environment.setParentChromosomesSurviveCount(INITIAL_PARENT_CHROMOSOMES_SURVIVE_COUNT);
