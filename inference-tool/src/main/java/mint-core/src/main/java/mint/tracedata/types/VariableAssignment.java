...
public abstract class VariableAssignment<T> {
	...
	private static List<Double> doubleValues = new ArrayList<Double>();
	private static List<List> listValues = new ArrayList<List>();
	private static List<Boolean> booleanValues = new ArrayList<Boolean>();
	
	public abstract List<T> getValues();
	
	public static void addValue(Object v) {
		if (v != null) {
			if (v instanceof String && !stringValues.contains(v))
				stringValues.add((String) v);
			else if (v instanceof Double && !doubleValues.contains(v))
				doubleValues.add((Double) v);
			else if (v instanceof Integer && !intValues.contains(v))
				intValues.add((Integer) v);
			else if (v instanceof List && !listValues.contains(v))
				listValues.add((List) v);
			else if (v instanceof Boolean && !booleanValues.contains(v)) 
				booleanValues.add((Boolean) v);
		}
	}
...
//            assert(from.contains(value));
	...
	public static List<List> getListValues() {
		return listValues;
	}
